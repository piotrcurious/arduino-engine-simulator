...
void EngineSimulator::update_dynamics() {
    ...
    float target_map = Physics::ATMOSPHERIC_PRESSURE_PA * (throttle_frac + (1.0 - throttle_frac) * 0.3);
    target_map -= (engine_rpm / 8000.0f) * 70000.0f * (1.0 - throttle_frac);
    manifold_pressure_pa = 0.95 * manifold_pressure_pa + 0.05 * constrain(target_map, 25000.0f, Physics::ATMOSPHERIC_PRESSURE_PA);
    ...
    for (int i = 0; i < Engine::NUM_CYLINDERS; ++i) {
        ...
        if (cylinders[cyl_idx].ignition_fired && phase_deg >= 0.0f && phase_deg <= 180.0f) {
            ...
            if (fuel_mass <= Engine::MIN_FUEL_MASS_KG && throttle_frac > 0.1) {
            ...
            float torque_curve = sin(phase_deg * (Physics::PI_VAL / 180.0f));
        ...
        if (!cylinders[cyl_idx].ignition_fired && phase_deg < 5.0f && engine_rpm > 500) {
    ...
    float coulomb_friction = (flywheel_speed_dps > 0) ? Physics::STATIC_FRICTION_NM : (flywheel_speed_dps < 0 ? -Physics::STATIC_FRICTION_NM : 0);
    ...
    flywheel_speed_dps += angular_accel_rad_s2 * (180.0f / Physics::PI_VAL) * Config::TIME_STEP_S;
    if (flywheel_speed_dps < 0) flywheel_speed_dps = 0;
...
void EngineSimulator::update_outputs() {
    int lambda_pwm = map(constrain(lambda_overall, 0.5f, 1.5f), 0.5f, 1.5f, 0, 255);
...
void EngineSimulator::write_telemetry() {
    ...
    if (millis() - last_print_ms > 250) {
        ...
        Serial.print(" | MAP: "); Serial.print(manifold_pressure_pa / 1000.0f, 1);
        Serial.print(" kPa | Torque: "); Serial.print(flywheel_torque_nm, 2);
        Serial.print(" Nm | Lambda: "); Serial.println(lambda_overall, 2);
    }
}
...
void setup() {
    Serial.begin(115200);
    Serial.println("Final Engine Simulator Initializing...");
