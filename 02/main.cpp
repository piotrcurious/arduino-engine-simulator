...
//
// Improved version with tuning panel, flywheel visualization,
// and oscilloscope for injection/ignition.
// =============================================================
...
// ═══════════════════════════════════════════════════════════
// FlywheelWidget – Shows rotating flywheel and crank angle
// ═══════════════════════════════════════════════════════════
...
public:
    ...
    void draw() override {
        ...
        for (int i=0; i<60; ++i) {
            if (i >= 58) continue; // missing teeth
            float a = i * 6.0f;
...
// ═══════════════════════════════════════════════════════════
// ScopeWidget – Oscilloscope for INJ and IGN
// ═══════════════════════════════════════════════════════════
...
static void throttle_cb(Fl_Widget* w, void*) {
    ...
    MockHW::analog_in[0] = (int)(pct * 10.23);
}

static void load_cb(Fl_Widget* w, void*) {
    double pct = ((Fl_Value_Slider*)w)->value();
    MockHW::analog_in[1] = (int)((pct/100.0)*511.5 + 511.5);
}

static void start_cb(Fl_Widget* w, void*) {
    bool pressed = ((Fl_Toggle_Button*)w)->value();
    MockHW::digital_io[Pins::START_BUTTON] = pressed ? ARD_LOW : ARD_HIGH;
...
static void tune_cb(Fl_Widget*, void*) {
    ...
    c.inj_flow_rate    = (float)ts_flow->value() / 10000.0f; // Scale back
    {
        std::lock_guard<std::mutex> lk(g_sim_mutex);
...
int main(int argc, char** argv) {
    MockHW::digital_io[Pins::START_BUTTON] = ARD_HIGH;
    MockHW::analog_in[0] = 0;
    MockHW::analog_in[1] = 511;
    {
        std::lock_guard<std::mutex> lk(g_sim_mutex);
        g_engine.setup();
        g_engine.set_config(EngineSimulator::TunableConfig()); // Load defaults
    }

    Fl::scheme("gtk+");
    Fl_Double_Window* win = new Fl_Double_Window(1000, 700, "Advanced Engine Simulator");
    ...
    Fl_Tabs* tabs = new Fl_Tabs(5, 5, 990, 660);
    {
        ...
        add_tuner("Peak Torque (Nm)", 10.0, 200.0, def.peak_torque, ts_torque);
        add_tuner("Inj Flow Rate (x10^-4 kg/s)", 0.1, 10.0, def.inj_flow_rate * 10000.0f, ts_flow);
