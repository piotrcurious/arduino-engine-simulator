...
#include <TimerOne.h>
#include <math.h>
#include <avr/interrupt.h> // for ISR macros (normally included via Arduino)
...
void EngineSimulator::setup() {
  ...
  for (int i = 0; i < Engine::NUM_CYLINDERS; ++i) {
    pinMode(Pins::INJ[i], INPUT); // external MCU drives these; keep INPUT to avoid forcing levels
  ...
  // Enable PCINT1 group (A0..A5), mask A0..A3 explicitly.
  // Use _BV for clarity and portability of the bit selection.
  PCICR |= _BV(PCIE1);
...
void setup() {
  Serial.begin(115200);
  #if defined(USBCON)
  while (!Serial) { } // only block on boards with USB CDC (Leonardo, Micro) so we don't hang on UNO
  #endif
